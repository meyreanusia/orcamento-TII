...
import apiModalidadeAplicacao from "../../services/apiModalidadeAplicacao.js";
...
export default function ModalidadeAplicacao() {
  ...
  const {  handleEditar, handleExcluir, handleBuscar, handleSubmit } = apiModalidadeAplicacao();
    ...
    handleSubmit(codigoNumber, nome)
      .then((resposta) => {
        if (resposta && resposta.ok) {
          ...
          console.error("Falha ao criar modalidade aplicação:", resposta);
          alert(
            "Erro ao criar Modalidade Aplicação! \n o código já existe na nossa base de dados."
          );
          setShowPopup(false);
          throw new Error("Erro ao criar a Modalidade Aplicação");
        }
      })
      .then((dadoCriado) => {
        alert("Modalidade Aplicação criada com sucesso");
        setDados([...dados, dadoCriado]);
        setShowPopup(false);

      })
      .catch((error) => {
        console.error("Erro ao criar modalidade aplicação:", error);
      });
  }
  ...
  return (
    <main className={`acao ${showPopup ? "popup-visivel" : ""}`}>
      <div className="container-tabela">
        <h1 className="titulo-pagina">Modalidade Aplicação</h1>
      ...
      {showPopup && (
        <Formulario handleSubmit={cadastrar}>
          <InputOrcamento
            titulo={"Cadastrar Modalidade Aplicação"}
            exibirInput={true}
